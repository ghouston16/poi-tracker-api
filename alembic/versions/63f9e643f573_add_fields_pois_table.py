...
def upgrade():
    ...
    op.create_table('users', sa.Column('id', sa.Integer(), primary_key=True, nullable=False), sa.Column("email", sa.String(), nullable=False), 
   ...
   ,)
    ...
    ,)
    op.add_column('pois', sa.Column('category', sa.String(), nullable=True),)
    op.add_column('pois', sa.Column('created_at',sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('NOW()')),)
    op.add_column('pois', sa.Column('published', sa.Boolean(), server_default='TRUE'))
    # Add creator column
    op.add_column('pois', sa.Column('creator', sa.Integer(), nullable=False),)
    # Make creator (user.id) FK
    op.create_foreign_key('pois_users_fk', source_table="pois", referent_table="users", local_cols=[
                          'creator'], remote_cols=['id'], ondelete="CASCADE")
    pass
